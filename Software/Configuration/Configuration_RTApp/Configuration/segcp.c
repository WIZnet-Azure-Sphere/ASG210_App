...
{
#define DEBUG_PARSE_SEGCP

    uint8_t** pcmd;
    uint8_t cmdnum = 0;
    uint8_t i;

#ifdef DEBUG_PARSE_SEGCP
    printf("Start DEBUG_PARSE_SEGCP\r\n");
#endif
...
#ifdef DEBUG_PROC_SEGCP
    printf("Start DEBUG_PROC_SEGCP\r\n");
#endif
...
// 20201028 taylor
        if((cmdnum = parse_SEGCP(treq, param)) != SEGCP_UNKNOWN)
        {
            ...
            if(*param == 0)
            {
                ...
                {
                case SEGCP_MC:
                    sprintf(trep,"%02X:%02X:%02X:%02X:%02X:%02X",
                ...
                case SEGCP_VR:
                    ...
                    break;
                case SEGCP_MN:
                    sprintf(trep,"%s", dev_config->module_name);
                    break;
                case SEGCP_IM:
                    sprintf(trep,"%d", dev_config->options.dhcp_use);	// 0:STATIC, 1:DHCP (PPPoE X)
                    break;
                case SEGCP_OP:
                    sprintf(trep,"%d", dev_config->network_info[0].working_mode); // opmode
                    break;
#if 0
// 20201119 taylor
                case SEGCP_DD:
                    sprintf(trep,"%d", 0); // Not used
                    break;
                case SEGCP_PO:
                    sprintf(trep,"%d", 0); // Not used
                    break;
                case SEGCP_CP:
                    sprintf(trep,"%d", dev_config->options.pw_connect_en);
                    break;
                case SEGCP_DG:
                    sprintf(trep,"%d", dev_config->serial_info[0].serial_debug_en);
                    break;
                case SEGCP_KA:
                    sprintf(trep,"%d", dev_config->network_info[0].keepalive_en);
                    break;
                case SEGCP_KI:
                    sprintf(trep,"%d", dev_config->network_info[0].keepalive_wait_time);
                    break;
                case SEGCP_KE:
                    sprintf(trep,"%d", dev_config->network_info[0].keepalive_retry_time);
                    break;
                case SEGCP_RI:
                    sprintf(trep,"%d", dev_config->network_info[0].reconnection);
                    break;
#endif
                case SEGCP_LI:
...
// 20201119 taylor
                ...
                case SEGCP_DP:
                    sprintf(trep,"%d", DEVICE_DDNS_PORT); // Not used
                    break;
                ...
                case SEGCP_DH:
                    ...
                    break;
                case SEGCP_LP:
                    sprintf(trep, "%d", dev_config->network_info[0].local_port);
                    break;
                case SEGCP_RP:
                    sprintf(trep, "%d", dev_config->network_info[0].remote_port);
                    break;
                case SEGCP_RH:
                    ...
                    break;
                case SEGCP_BR:
                    sprintf(trep, "%d", dev_config->serial_info[0].baud_rate);
                    break;
                case SEGCP_DB:
                    sprintf(trep, "%d", dev_config->serial_info[0].data_bits);
                    break;
                case SEGCP_PR:
                    sprintf(trep, "%d", dev_config->serial_info[0].parity);
                    break;
                case SEGCP_SB:
                    sprintf(trep, "%d", dev_config->serial_info[0].stop_bits);
                    break;
                case SEGCP_FL:
                    sprintf(trep, "%d", dev_config->serial_info[0].flow_control);
                    break;
                case SEGCP_IT:
                    sprintf(trep, "%d", dev_config->network_info[0].inactivity);
                    break;
                case SEGCP_PT:
                    sprintf(trep, "%d", dev_config->network_info[0].packing_time);
                    break;
                case SEGCP_PS:
                    sprintf(trep, "%d", dev_config->network_info[0].packing_size);
                    break;
                case SEGCP_PD:
                    sprintf(trep, "%02X", dev_config->network_info[0].packing_delimiter[0]);
                    break;
                case SEGCP_TE:
                    sprintf(trep, "%d", dev_config->options.serial_command);
                    break;
                case SEGCP_SS:
                    sprintf(trep, "%02X%02X%02X", dev_config->options.serial_trigger[0], dev_config->options.serial_trigger[1], dev_config->options.serial_trigger[2]);
                    break;
                case SEGCP_NP:
...
// 20201119 taylor
                ...
                case SEGCP_AB: // Added command to enter AppBoot mode: AB
                    ...
                    break;
                case SEGCP_TR:
                    sprintf(trep, "%d", dev_config->options.tcp_rcr_val);
                    break;
                case SEGCP_BU:
                    ret |= SEGCP_RET_ERR_NOPARAM;
                    break;
            ...
            {
#ifdef DEBUG_PROC_SEGCP
                printf("Start gSEGCPPRIVILEGE & (SEGCP_PRIVILEGE_SET|SEGCP_PRIVILEGE_WRITE)\r\n");
#endif
...
#ifdef DEBUG_PROC_SEGCP
                printf("End gSEGCPPRIVILEGE & (SEGCP_PRIVILEGE_SET|SEGCP_PRIVILEGE_WRITE)\r\n");
#endif
            }
            else
            {
...
#if 1
// 20201028 taylor
            len = sock_recvfrom(SEGCP_UDP_SOCK, treq, len, destip, &destport);
#else
...
#if 1
// 20201028 taylor
#ifdef DEBUG_PROC_SEGCP_UDP
            printf("Start DEBUG_PROC_SEGCP_UDP\r\n");
            printf("UDP Received len = %d\r\n", len);
...
// 20201028 taylor
#ifdef DEBUG_PROC_SEGCP_UDP
                    printf("Start gSEGCPPRIVILEGE & SEGCP_PRIVILEGE_SET Matched\r\n");
#endif
#endif
...
#if 1
// 20201028 taylor
                            sock_sendto(SEGCP_UDP_SOCK, segcp_rep, 14+strlen(tpar)+strlen(trep), "\xFF\xFF\xFF\xFF", destport);
#else
...
// 20201028 taylor
#ifdef DEBUG_PROC_SEGCP_UDP
                    printf("End gSEGCPPRIVILEGE & SEGCP_PRIVILEGE_SET Matched\r\n");
#endif
#endif
                }
                else
...
// 20201028 taylor
#ifdef DEBUG_PROC_SEGCP_UDP
                    printf("End DEBUG_PROC_SEGCP_UDP\r\n");
#endif
#endif
...
#if 1
// 20201028 taylor
        if(wiz_socket(SEGCP_UDP_SOCK, Sn_MR_UDP, DEVICE_SEGCP_PORT, SOCK_IO_NONBLOCK) == SEGCP_UDP_SOCK) //191213 irina  add SOCK_IO_NONBLOCK
        {
#if 1
// 20201028 taylor
            printf(" > SEGCP:UDP:STARTED\r\n");
#else
...
#if 1
// 20201028 taylor
            len = sock_recv(SEGCP_TCP_SOCK,treq,len);
#else
...
#if 1
// 20201028 taylor
                            sock_send(SEGCP_TCP_SOCK, segcp_rep, 14+strlen(tpar)+strlen(trep));
#else
...
#if 1
// 20201028 taylor
        sock_disconnect(SEGCP_TCP_SOCK);
#else
...
#if 1
// 20201028 taylor
        if(wiz_socket(SEGCP_TCP_SOCK, Sn_MR_TCP, DEVICE_SEGCP_PORT, SF_TCP_NODELAY|SOCK_IO_NONBLOCK) == SEGCP_TCP_SOCK)
        {
