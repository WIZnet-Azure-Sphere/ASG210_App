...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    // Open USI_MT3620_BT_COMBO_PIN63_GPIO6 GPIO, set as output with value GPIO_Value_Low for using RS232 instead of RS485
    Log_Debug("Opening USI_MT3620_BT_COMBO_PIN63_GPIO6 as output.\n");
    gpioNRS232Fd = GPIO_OpenAsOutput(USI_MT3620_BT_COMBO_PIN63_GPIO6, GPIO_OutputMode_PushPull, GPIO_Value_Low);
    if (gpioNRS232Fd == -1) {
        Log_Debug("ERROR: Could not open button GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_OpenButton;
    }

    // Open USI_MT3620_BT_COMBO_PIN65_GPIO4 GPIO, set as output with value GPIO_Value_High for using RXD3 instead of I2C_SDA
    Log_Debug("Opening USI_MT3620_BT_COMBO_PIN65_GPIO4 as output.\n");
    gpioNSCL = GPIO_OpenAsOutput(USI_MT3620_BT_COMBO_PIN65_GPIO4, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (gpioNSCL == -1) {
        Log_Debug("ERROR: Could not open button GPIO: %s (%d).\n", strerror(errno), errno);
        return ExitCode_Init_OpenButton;
    }
    ...
    uartFd = UART_Open(USI_MT3620_BT_COMBO_ISU3_UART, &uartConfig);
    if (uartFd == -1) {
