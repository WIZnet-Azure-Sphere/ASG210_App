...
static ExitCode InitPeripheralsAndHandlers(void)
{
    ...
    ledBlinkRateButtonGpioFd = GPIO_OpenAsInput(USI_MT3620_BT_COMBO_PIN55_GPIO13);
    if (ledBlinkRateButtonGpioFd == -1) {
        Log_Debug("ERROR: Could not open USI_MT3620_BT_COMBO_PIN55_GPIO13: %s (%d).\n", strerror(errno), errno);
    ...
    // Open USI_MT3620_BT_COMBO_PIN28_GPIO41 GPIO, set as output with value GPIO_Value_High (off), and set up a timer to
    // blink it
    Log_Debug("Opening USI_MT3620_BT_COMBO_PIN28_GPIO41 as output.\n");
    blinkingLedGpioFd = GPIO_OpenAsOutput(USI_MT3620_BT_COMBO_PIN28_GPIO41, GPIO_OutputMode_PushPull, GPIO_Value_High);
    if (blinkingLedGpioFd == -1) {
        Log_Debug("ERROR: Could not open USI_MT3620_BT_COMBO_PIN28_GPIO41 GPIO: %s (%d).\n", strerror(errno), errno);
